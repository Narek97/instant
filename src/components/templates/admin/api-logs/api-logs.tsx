...
import { ApiLogType, ErrorLogType } from '@/ts/types/admin'
...
const ApiLogs = () => {
  const [apiLogs, setApiLogs] = useState<Array<ErrorLogType> | null>(null)
              ...
              {dataApiLogs.count ? (
                <>
                  <CustomTable
                    columns={columns}
                    rows={rows}
                    rowFunction={(row: ApiLogType) => <ApiLogsRow row={row} />}
                  />
